...
export function classifyInterviewQuestion(text: string): InterviewDimension | null {
  ...
  if (/(?:接下来|下面|然后|除了|想(?:再|单独)?了解|聊聊|先问问).{0,100}(?:项目|活动|大作业|课程设计|课程论文|毕业设计|竞赛|比赛|大赛|模拟法庭|法律援助|社会实践|公益|志愿|社团|学生组织).{0,160}[？?]/.test(withoutTags) ||
      /(?:模拟法庭|法律援助|竞赛|比赛|大赛|个人项目|课程项目|课程设计|课程论文|毕业设计|社会实践|公益活动|志愿活动|社团工作).{0,120}(?:负责|角色|做了什么|困难|挑战|结果|成果).{0,40}[？?]/.test(withoutTags)) {
    return 'project'
  }
  ...
  if (/课程(?:中|里|之外|以外)?.{0,30}(?:大作业|课程项目|课程设计|课程论文|毕业论文|毕业设计)|(?:大作业|课程设计|课程论文|毕业设计).{0,24}(?:有没有|哪|介绍|讲|做过)|(?:竞赛|比赛|大赛|个人项目|社团|志愿|社会实践).{0,24}(?:有没有|做过|参加过)/.test(clean)) return 'project'
