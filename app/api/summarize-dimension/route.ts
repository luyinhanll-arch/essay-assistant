...
export async function POST(req: Request) {
  try {
    ...
    const DIM_EXCLUDE: Record<string, string> = {
      ...
      research:   '**包含**：在导师/实验室框架下的正式科研课题（包括**横向课题、纵向课题**）、独立撰写并发表的学术论文/研究报告。**不要**包含：课程作业/课程项目（无发表）、竞赛、社团活动、实习岗位本身',
    }
...
- 即使只有一段经历，也**必须**有标题行
- 每个要点写成一句完整的自然语句（15-40 字），**统一用第二人称"你"**，把角色/做了什么、方法/思路、遇到的挑战、如何解决、结果等叙事要素**有机融入句子里**，而不是拆成独立标签行——信息量多就多写一句，信息量少就精炼成一句，**不得使用省略号（…或...）**
...
- 合并后的要点去重，保留信息量最大的表述
- 确实是不同经历的组**保持不变，不要合并**
- **特别注意**：同一课程/机构下的不同角色（如"课程项目"和"助教"）是两件不同的事，**不得合并**，即使课程名相同
