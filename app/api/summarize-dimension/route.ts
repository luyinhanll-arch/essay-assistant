...
export async function POST(req: Request) {
  try {
    const {
      ...
      format = 'structured',
      relatedSummaries = {},
    }: { dimension: string; messages: Message[]; format?: 'structured' | 'paragraph'; relatedSummaries?: Record<string, string> } = await req.json()
    ...
    if (format === 'paragraph') {
      ...
      if (isMultiEntry) {
        ...
        userPrompt = `请扫描以下完整访谈对话（包括所有话题），找出申请者在【${dimensionLabel}】方面**曾经提到过的所有经历**——无论是在正式讨论该话题时提及的，还是在其他话题中顺带提及的。
${claimedBlock ? `\n${claimedBlock}\n` : ''}
对话内容：
${conversationText}
...
- 每句必须具体：提到真实名称、数据（时长/规模/成果）；访谈中没提到的内容不要编造
...
· 目标院校的实验室在对话生成方向有持续发表，与研究兴趣高度匹配`
      ...
      if (isMultiEntry) {
        userPrompt = `请扫描以下完整访谈对话（包括所有话题），找出用户在【${dimensionLabel}】方面**曾经提到过的所有经历**，每段单独成组。
${claimedBlock ? `\n${claimedBlock}\n` : ''}
对话内容：
${conversationText}
